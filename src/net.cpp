...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Nefcoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"nefcointools.com", "149.28.203.157"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Nefcoin is probably already running."), addrBind.ToString().c_str());
        else
