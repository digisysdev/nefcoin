...
#if defined(NDEBUG)
# error "Nefcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x155b5f83d3d2765c8e47bc3c30a29194089da67d72fadcc1b127c62f078bac59");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Nefcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Nefcoin Signed Message:\n";
...
{
    // Nefcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Nefcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Nefcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Nefcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Nefcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Nefcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x155b5f83d3d2765c8e47bc3c30a29194089da67d72fadcc1b127c62f078bac59");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is nefcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xb95c8484134e62e04248535d2b5109cd29d6ffb7006514411d315fe1bfc9b332"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Nefcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("NefcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("NefcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static NefcoinMiner(CWallet *pwallet)
{
    printf("NefcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("nefcoin-miner");
    ...
    try { loop {
        ...
        printf("Running NefcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("NefcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&NefcoinMiner, pwallet));
}
